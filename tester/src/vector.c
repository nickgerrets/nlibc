...
void test_vector(void)
{
	...
	TEST("n_vector_new()", vector.mem != NULL);

	n_vector_add(&vector, &(int){4});
	TEST("n_vector_add()", DREF(int, vector.mem) == 4);

	{
		int const _array[] = {9, 8, 7, 6, 5, 4, 3, 2, 1};
		n_vector_add_array(&vector, _array, sizeof(_array) / sizeof(int));
		TEST("n_vector_add_array()", memcmp(_array, CAST(int*, vector.mem) + 1, sizeof(_array) / sizeof(int)) == 0);
	}
...

	{
		...
		TEST("n_vector_search() non-existant", index == vector.curr_count);
	}
