...
void tester_lists(void)
{
	NL; {
		...
		list = NULL;
		// t_vector int_vec = n_vector_create(sizeof(int));
		// n_vector_push_back_array(&int_vec, _array, sizeof(_array) / sizeof(int));

		n_putstr_endl("n_list_push_front() an entire array");
		for (size_t i = 0; i < sizeof(_array) / sizeof(int); ++i)
			n_list_push_front(&list, n_list_create(_array + i));
